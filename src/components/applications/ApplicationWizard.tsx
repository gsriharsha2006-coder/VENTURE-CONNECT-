...
function qualityFieldLabel(field: string) {
  return fields.find(([key]) => key === field)?.[1] ?? field.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, (value) => value.toUpperCase());
}
...
export function ApplicationWizard({ opportunity, workspaces, founderName }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {step === 3 && quality ? <Card>
        <div className="flex flex-wrap items-start justify-between gap-4"><CardHeader eyebrow="Application Quality Check" title={`${quality.score}/100`} /><Badge tone={quality.status === "ready_to_submit" ? "green" : quality.status === "eligibility_mismatch" ? "red" : "amber"}>{qualityStatusLabels[quality.status]}</Badge></div>
        <p className="text-sm leading-6 text-slate-600">{quality.summary}</p>
        <p className="mt-2 text-xs leading-5 text-slate-500">This score measures application completeness and clarity. It does not predict startup success, investment potential, or funding outcomes.</p>
        {quality.eligibilityMismatches.length ? <section className="mt-5 rounded-lg border border-rose-200 bg-rose-50 p-4"><h2 className="text-sm font-semibold text-rose-950">Eligibility Mismatch</h2><div className="mt-3 space-y-3">{quality.eligibilityMismatches.map((item) => <dl key={`${item.field}-${item.expected}`} className="grid gap-1 text-sm sm:grid-cols-[150px_1fr]"><dt className="font-semibold text-rose-900">Requirement</dt><dd>{item.requirement}</dd><dt className="font-semibold text-rose-900">Expected</dt><dd>{item.expected}</dd><dt className="font-semibold text-rose-900">Application</dt><dd>{item.actual}</dd></dl>)}</div></section> : null}
        ...
        {quality.majorIssues.length ? <section className="mt-5"><h2 className="text-sm font-semibold">Major Issues</h2><ol className="mt-2 list-decimal space-y-2 pl-5 text-sm text-slate-600">{quality.majorIssues.slice(0, 3).map((item) => <li key={item}>{item}</li>)}</ol></section> : null}
        {quality.corrections.length ? <section className="mt-5"><h2 className="text-sm font-semibold">Field-specific Corrections</h2><div className="mt-2 space-y-3">{quality.corrections.map((item, index) => <div key={`${item.field}-${index}`} className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><h3 className="font-semibold">{qualityFieldLabel(item.field)}</h3><p className="mt-1"><strong>Issue:</strong> {item.issue}</p><p className="mt-1"><strong>Correction:</strong> {item.correction}</p></div>)}</div></section> : null}
        {quality.status === "manual_review" && quality.manualReviewReason ? <StatusMessage className="mt-5"><strong>Manual Review:</strong> {quality.manualReviewReason}</StatusMessage> : null}
        ...
        <div className="mt-6 flex flex-wrap justify-between gap-3">{quality.status !== "eligibility_mismatch" ? <Button variant="secondary" onClick={() => setStep(2)} disabled={checkCount >= 2}><ChevronLeft size={16} />Edit and Recheck</Button> : <Button variant="secondary" onClick={() => setStep(2)}><ChevronLeft size={16} />Edit Application</Button>}{quality.status === "ready_to_submit" ? <Button onClick={() => setStep(4)}><ChevronRight size={16} />Submit Application</Button> : null}</div>
      </Card> : null}
